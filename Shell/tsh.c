...
pid_t getfgpid(struct job_t* jobs);
...
void eval(char* cmdline)
{
	...
	// TODO: Implement two process piping.

	// Declare a signal set.
	sigset_t sigSet;
	// Attempt to execute as built-in, otherwise enter branch.
	if (!builtin_cmd(argv1)) {
		// Initialize signal set mask and block parent from receiving SIGINT, SIGTSTP, & SIGCHLD.
		sigemptyset(&sigSet);
		...
		if ((pid = fork()) < 0) {
			...
			// Process in BG if parseline returned true.
			if (bg) {
				// Add BG job.
			...
			// Otherwise process in FG and wait.
			else {
...
{
	char* cmd = argv[0];
	// If quit command, simply exit.
	if (strcmp(cmd, "quit") == 0)
		exit(0);
	// If jobs command, list jobs and return true.
...
void do_bg(int jid)
{	
	job_t* jobPtr = getjobjid(jobs, jid);
	// Resume process with SIGCONT.
	if (kill(-(jobPtr -> pid), SIGCONT) < 0) {
		fprintf(stderr, "%d: Error resuming process with SIGCONT", jid);
		return;
	}
	// Set process to background.
	printf("[%d] (%d) %s", jid, jobPtr -> pid, jobPtr -> cmdline);
	jobPtr -> state = BG;
	return;
}
...
void do_fg(int jid)
{
	...
	// Set process to background and wait.
	jobPtr -> state = FG;
	waitfg(jobPtr -> pid);
	return;
}
...
void waitfg(pid_t pid)
{
	job_t* jobPtr = getjobpid(jobs, pid);
	while (jobPtr -> state == FG)
		sleep(1);
	return;
}
...
void sigchld_handler(int sig)
{
	pid_t chldPID;
	int status;
	while ((chldPID = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
		job_t* jobPtr = getjobpid(jobs, chldPID);
		if (WIFEXITED(status)) {
			jobPtr -> state = UNDEF;
			deletejob(jobs, chldPID);
		}
		else if (WIFSIGNALED(status)) {
			printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(chldPID), chldPID, WTERMSIG(status));
			deletejob(jobs, chldPID);
		}
		else if (WIFSTOPPED(status)) {
			printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(chldPID), chldPID, WSTOPSIG(status));
			jobPtr -> state = ST;
		}
	}
	return;
...
void sigint_handler(int sig)
{
	int fgPID = getfgpid(jobs);
	if (fgPID != 0) {
		kill(-fgPID, SIGINT);
		if (sig < 0) {
			printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(fgPID), fgPID, (-sig));
...
/* getfgpid - Finds and returns FG job from list. */
pid_t getfgpid(struct job_t* jobs) {
	int idx;
	for (idx = 0; idx < MAXJOBS; idx++) {
		if (jobs[idx].state == FG)
			return jobs[idx].pid;
	}
	return 0;
}
